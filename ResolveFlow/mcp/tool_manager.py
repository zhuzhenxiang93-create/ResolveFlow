...
本模块的答案：
  1. 查询改写（Query Rewriting）—— 用 LLM 把用户原始问题扩写成多个角度的子查询，
     再合并去重，解决"召回不全"问题。
  2. 结果重排（Reranking）—— 对召回结果用专用 Qwen3 Reranker 打分排序，
     解决"召回不好/排序差"问题。
...
class MCPToolManager:
    ...
    def __init__(self, api_key: str, base_url: Optional[str] = None, model: str = "claude-3-5-sonnet-20241022", provider: Optional[str] = None, reranker: Optional[Qwen3Reranker] = None):
        ...
        *,
        ...
        if use_cache and tool.cache_ttl > 0:
            ...
            if cached is not None:
                cached_data, cached_reranked, cached_degradations = cached
                tool.stats.total += 1
                ...
                return ToolResult(
                    ...
                    degradations=cached_degradations,
                )
        ...
        try:
            ...
            if rerank_top_k > 0 and tool.supports_rerank and isinstance(data, list):
                ...
                if not reranked and candidate_total > rerank_top_k:
                    degradations.append("qwen3_rerank_unavailable")
...
返回 JSON 数组，例如: ["子查询1", "子查询2", "子查询3"]"""
        ...
        try:
            ...
            # 原始查询也保留，去重
            return list(dict.fromkeys([query] + queries)), True
        except Exception as ex:
    ...
    async def search_with_rewrite(
        ...
        degradations = [] if rewrite_succeeded else ["query_rewrite_unavailable"]
                ...
                for item in r.data:
                    if isinstance(item, dict):
                        key = item.get("chunk_id") or hashlib.md5(
                            f"{item.get('document_id', '')}:{item.get('content', '')}".encode()
                        ).hexdigest()
                    else:
        ...
        reranked, rerank_succeeded = await self._rerank(query, merged, top_k)
        if not rerank_succeeded and len(merged) > top_k:
            degradations.append("qwen3_rerank_unavailable")
        return ToolResult(
            success=True,
            data=reranked,
            tool_name=tool_name,
            reranked=rerank_succeeded,
            degradations=degradations,
        )
    ...
    async def _rerank(self, query: str, items: List[Any], top_k: int) -> Tuple[List[Any], bool]:
        """
        用专用 qwen3-rerank 对召回结果重新打分排序。

        解决问题：向量检索的相似度分数不等于"对用户有用"，
        专用相关性模型联合读取 query 和候选，比分数直接相加更适合精排。
        """
        if len(items) <= 1:
            return items, False
        ...
        if reranker is None:
            logger.warning("qwen3-rerank 未配置，降级为 RRF/原始顺序")
            return items[:top_k], False
        try:
            results = await reranker.rerank(self._clean_text(query), items)
            valid_order = [result.index for result in results]
            seen = set(valid_order)
            # 服务端返回不完整时，按原 RRF 顺序补回遗漏候选。
            valid_order.extend(index for index in range(len(items)) if index not in seen)
            score_by_index = {result.index: result.score for result in results}
            reranked: List[Any] = []
            for index in valid_order[:top_k]:
                item = items[index]
                if isinstance(item, dict):
                    item = dict(item)
                    if index in score_by_index:
                        item["rerank_score"] = score_by_index[index]
                    item["rerank_model"] = reranker.model
                reranked.append(item)
            return reranked, True
        except Exception as ex:
            logger.warning(f"qwen3-rerank 失败，降级为 RRF/原始顺序: {ex}")
            return items[:top_k], False
    ...
    def _get_cache(
        self, name: str, params: Dict, rerank_top_k: int = 0
    ) -> Optional[Tuple[Any, bool, List[str]]]:
        key = self._cache_key(name, params, rerank_top_k)
        if key in self._cache:
            data, expire_at, reranked, degradations = self._cache[key]
            if time.monotonic() < expire_at:
                return data, reranked, list(degradations)
            del self._cache[key]
        return None
