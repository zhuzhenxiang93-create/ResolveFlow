...
- `docker-compose.yml` 里还有一个 `postgres` 服务（`profiles: ["pg-poc"]`），默认不随
  `docker compose up -d` 启动，只在验证 SQLite→PostgreSQL 迁移 POC 时用 `make pg-poc-up` 手动拉起，
  不在这张图的主链路里（见 `wiki/postgres-migration-plan.md`）。
